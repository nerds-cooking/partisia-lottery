...
export class LotteryApiGenerated {
  ...
  public deserializeContractState(_input: AbiInput): ContractState {
    ...
    const userAccounts: AvlTreeMap<BlockchainAddress, SecretVarId> =
      new AvlTreeMap(
        userAccounts_treeId,
        this._client,
        this._address,
        (userAccounts_key) =>
          AbiByteOutput.serializeLittleEndian((userAccounts_out) => {
            userAccounts_out.writeAddress(userAccounts_key);
          }),
        (userAccounts_bytes) => {
          const userAccounts_input =
            AbiByteInput.createLittleEndian(userAccounts_bytes);
          const userAccounts_key: BlockchainAddress =
            userAccounts_input.readAddress();
          return userAccounts_key;
        },
        (userAccounts_bytes) => {
          const userAccounts_input =
            AbiByteInput.createLittleEndian(userAccounts_bytes);
          const userAccounts_value: SecretVarId =
            this.deserializeSecretVarId(userAccounts_input);
    ...
    const uaAccountKeyMap: AvlTreeMap<BN, BlockchainAddress> = new AvlTreeMap(
      ...
      this._address,
      (uaAccountKeyMap_key) =>
        AbiByteOutput.serializeLittleEndian((uaAccountKeyMap_out) => {
          uaAccountKeyMap_out.writeUnsignedBigInteger(uaAccountKeyMap_key, 16);
        }),
      (uaAccountKeyMap_bytes) => {
        const uaAccountKeyMap_input = AbiByteInput.createLittleEndian(
          uaAccountKeyMap_bytes
        );
        const uaAccountKeyMap_key: BN =
          uaAccountKeyMap_input.readUnsignedBigInteger(16);
      ...
      (uaAccountKeyMap_bytes) => {
        ...
        const uaAccountKeyMap_value: BlockchainAddress =
          uaAccountKeyMap_input.readAddress();
    ...
    const lotteryAccounts: AvlTreeMap<BN, SecretVarId> = new AvlTreeMap(
      ...
      this._address,
      (lotteryAccounts_key) =>
        AbiByteOutput.serializeLittleEndian((lotteryAccounts_out) => {
          lotteryAccounts_out.writeUnsignedBigInteger(lotteryAccounts_key, 16);
        }),
      (lotteryAccounts_bytes) => {
        const lotteryAccounts_input = AbiByteInput.createLittleEndian(
          lotteryAccounts_bytes
        );
        const lotteryAccounts_key: BN =
          lotteryAccounts_input.readUnsignedBigInteger(16);
      ...
      (lotteryAccounts_bytes) => {
        ...
        const lotteryAccounts_value: SecretVarId = this.deserializeSecretVarId(
          lotteryAccounts_input
        );
    ...
    const lotteries: AvlTreeMap<BN, LotteryState> = new AvlTreeMap(
      ...
      this._address,
      (lotteries_key) =>
        AbiByteOutput.serializeLittleEndian((lotteries_out) => {
          lotteries_out.writeUnsignedBigInteger(lotteries_key, 16);
        }),
      (lotteries_bytes) => {
        const lotteries_input =
          AbiByteInput.createLittleEndian(lotteries_bytes);
      ...
      (lotteries_bytes) => {
        const lotteries_input =
          AbiByteInput.createLittleEndian(lotteries_bytes);
        const lotteries_value: LotteryState =
          this.deserializeLotteryState(lotteries_input);
  ...
  public deserializeLotteryState(_input: AbiInput): LotteryState {
    ...
    if (secretStateId_isSome) {
      const secretStateId_option: SecretVarId =
        this.deserializeSecretVarId(_input);
    ...
    if (pendingSecretStateId_isSome) {
      const pendingSecretStateId_option: SecretVarId =
        this.deserializeSecretVarId(_input);
  ...
  public deserializeLotteryStatusPending(
    _input: AbiInput
  ...
  public deserializeWorkListItemPendingDrawWinner(
    _input: AbiInput
...
function serializeLotteryCreationSecret(
  _out: AbiOutput,
  _value: LotteryCreationSecret
): void {
...
function serializeLotteryTicketPurchaseSecret(
  _out: AbiOutput,
  _value: LotteryTicketPurchaseSecret
): void {
...
export function createAccount(
  accountKey: BN
): SecretInputBuilder<AccountCreationSecret> {
...
export function createLottery(
  ...
  entryCost: BN,
  prizePool: BN
): SecretInputBuilder<LotteryCreationSecret> {
  const _publicRpc: Buffer = AbiByteOutput.serializeBigEndian((_out) => {
  ...
  const _secretInput = (
    secret_input_lambda: LotteryCreationSecret
  ): CompactBitArray =>
    AbiBitOutput.serialize((_out) => {
...
export function purchaseTickets(
  lotteryId: BN
): SecretInputBuilder<LotteryTicketPurchaseSecret> {
  const _publicRpc: Buffer = AbiByteOutput.serializeBigEndian((_out) => {
    _out.writeBytes(Buffer.from('42', 'hex'));
    _out.writeUnsignedBigInteger(lotteryId, 16);
  });
  const _secretInput = (
    secret_input_lambda: LotteryTicketPurchaseSecret
  ): CompactBitArray =>
    AbiBitOutput.serialize((_out) => {
